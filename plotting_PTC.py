...
####################################################
PTC_data_file = 'Z:/Characterizations/Measurements_final//CDAVIS/PTC/DAVISHet640_ADCint_ptc_08_06_16-13_26_05/saved_variables/variables_DAVISHet640.npz'
...
if (plot_signal_exposure):
    ...
    plt.xlabel('Exposure Time ['+r'$\mu$'+'s]')
    ...
    ax1.set_ylabel('Spatiotemporal Mean Signal [DN]', color='b')
    for tl in ax1.get_yticklabels():
        tl.set_color('b')
    ax2.set_ylabel('Linearity Deviation [%]', color='r')
...
if (plot_PTC_linear_fit):
    ...
    plt.xlabel('Spatiotemporal Mean Signal [DN]')
...
if (plot_FPN):
    ...
    for this_area_x in range(len(frame_x_divisions)):
        for this_area_y in range(len(frame_y_divisions)):
            ...
            plt.text(350, 5.5, 'Total FPN', color='k')
            plt.text(350, 3.5, 'Row FPN', color='b')
            plt.text(100, 2, 'Column FPN', color='r')
            plt.annotate('Total FPN at 50% of saturation: ' + str(format(
...
if (plot_SNR):
    ...
    plt.figure()
    #plt.title('CDAVIS SNR vs Signal')
    for this_area_x in range(len(frame_x_divisions)):
        for this_area_y in range(len(frame_y_divisions)):
            SNR = 20 * np.log10(spatiotemporal_mean[:, this_area_y, this_area_x] / np.sqrt(
                temporal_var_DN2[:, this_area_y, this_area_x]))
            plt.plot(
            ...
            ind_100 = np.where(spatiotemporal_mean[:, this_area_y, this_area_x] >= 600.0)[0][0]
            plt.annotate(
    ...
    plt.ylabel('SNR [DB]')
    plt.ylim(0, 50)
    plt.savefig(output_dir + "APS_SNR_CDAVIS.pdf", format='pdf', bbox_inches='tight')
    plt.savefig(output_dir + "APS_SNR_CDAVIS.png", format='png', bbox_inches='tight', dpi=600)
